...
class PeakyLoss (nn.Module):
    ...
    def forward(self, repeatability, **kw):
        sali1, sali2 = repeatability#list
        # print("repeatability", type(repeatability[0]))#list, size() error, numpy() error, detach().numpy() list cannot detach
        # print("repeatability", repeatability[0].size())#repeatability torch.Size([8, 1, 192, 192])
        # print("forward one",type(sali1))
        # print("forward one",sali1.size())#[8,1,192,192] 8, batch_sizefor 1 picture
